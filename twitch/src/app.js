// ===== TwitchSave — 10. Twitch Video Downloader App =====

const ALLOWED_OPTION_VALUES = ['1080p', '720p', '360p', 'mp3']
const OPTION_LABELS = {
  '1080p': '1080p60 Full HD (Source)',
  '720p': '720p60 HD (High Quality)',
  '360p': '360p SD (Compact)',
  'mp3': 'MP3 Audio (Commentary / Music)',
}
...
const FRIENDLY_ERRORS = {
  400: 'Please enter a valid Twitch video, VOD, or clip link (e.g. twitch.tv/videos/... or clips.twitch.tv/...).',
  404: 'The requested video file could not be found.',
  413: 'The file is too large to process.',
  429: 'The server is busy. Please try again later.',
  500: 'This Twitch video could not be downloaded.',
  504: 'The download took too long and was stopped.',
}

// ── Twitch URL normalization & validation ─────────────────────────────────────
...
function isValidTwitchUrl(rawUrl) {
  ...
  try {
    ...
    if (host === 'twitch.tv' || host.endsWith('.twitch.tv')) {
      ...
      if (path.includes('/videos/') || path.includes('/clip/') || path.includes('/v/')) return true
    ...
    // Supported fallbacks
    if (['pinterest.com', 'pin.it', 'reddit.com', 'old.reddit.com', 'sh.reddit.com', 'm.reddit.com', 'redd.it', 'v.redd.it', 'twitter.com', 'x.com', 'youtube.com', 'youtu.be', 'snapchat.com', 'bilibili.com', 'b23.tv'].some(h => host === h || host.endsWith('.' + h))) {
      return !!u.pathname.replace('/', '').trim()
    }
    return false
  } catch {
...
<header class="site-header">
  <div class="header-inner">
    <a href="#home" class="logo-link" aria-label="10. Twitch Video Downloader home">
      <div class="logo-icon">${ICONS.twitch}</div>
      <span class="logo-text">10. <span class="logo-accent">Twitch</span></span>
    </a>
...
<main>
  ...
  <section id="home" class="hero">
    ...
    <div class="hero-content">
      <div class="hero-badge">
        ${ICONS.twitch}
        <span>#1 Free Twitch Video Downloader</span>
      </div>
      <h1>10. Twitch Video Downloader<br/><span class="twitch-accent">In HD Quality with Sound</span></h1>

      <div class="downloader-card" id="downloader-card">
        <div class="audio-highlight-bar">
          <span class="audio-tag">${ICONS.volume} <strong>Sound Guaranteed:</strong> Crystal Clear Twitch Stream Audio &amp; Video MP4</span>
        ...
        <form id="download-form" novalidate>
          <div class="input-row">
            <div class="url-input-wrap">
              <div class="twitch-icon" style="color: var(--twitch-purple); display: flex; align-items: center;">${ICONS.twitch}</div>
              <input
                type="url"
                id="url-input"
                name="url"
                placeholder="Paste Twitch VOD or clip link (e.g. twitch.tv/videos/... or clips.twitch.tv/...)"
                autocomplete="off"
                spellcheck="false"
                aria-label="Twitch video or clip URL"
              />
        ...
        <!-- Loading state: Progress line removed for clean fetch UX -->
        <div class="loading-area" id="loading-area" hidden>
          <div class="loading-spinner">
            <div class="spinner-ring"></div>
          </div>
          <div class="loading-text">
            <p id="loading-msg">Fetching Twitch video &amp; audio...</p>
            <p class="loading-sub">Extracting stream and audio tracks for full quality.</p>
          </div>
        </div>

        <!-- Twitch Video/Clip Info Preview Card -->
        <div class="reddit-preview-card" id="info-preview" hidden>
          <div class="tweet-card-header">
            <div class="reddit-avatar" style="background: var(--twitch-purple); box-shadow: 0 2px 8px var(--twitch-purple-glow);">${ICONS.twitch}</div>
            <div class="tweet-user-meta">
              <div class="tweet-user-row">
                <span class="subreddit-tag" id="info-subreddit" style="color: var(--twitch-purple); background: var(--twitch-purple-light); border-color: var(--twitch-purple-border);">Twitch Clip</span>
                <span class="tweet-author-name" id="info-author">Twitch Streamer</span>
              </div>
              <p class="tweet-text-content" id="info-title"></p>
            </div>
          </div>
          <div class="info-thumb-wrap">
            <img id="info-thumb" src="" alt="Twitch video preview" class="info-thumb" />
            <div class="info-thumb-overlay">
              <svg viewBox="0 0 24 24" fill="white" width="44" height="44">
                <circle cx="12" cy="12" r="11" fill="rgba(145, 70, 255, 0.85)"/>
        ...
        <div class="result-card" id="result-card" hidden aria-live="polite">
          <div class="result-header">
            ...
            <span>Your Twitch Video is Ready!</span>
          </div>
      ...
      <p class="hero-legal">
        For publicly accessible Twitch VODs, clips, and highlights. Compliant with copyright laws and platform terms.
      </p>
    </div>
  </section>
      ...
      <div class="format-chips">
        ...
        <div class="format-chip featured">
          ...
          <div>
            ...
            <span>Crisp &amp; Fast Stream Download</span>
          </div>
        ...
        <div class="format-chip">
          ...
          <div>
            ...
            <span>Pure Stream Audio / Commentary</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <section id="how-it-works" class="section">
    <div class="container">
      <div class="section-header">
        <h2>How to Download Twitch Videos &amp; Clips</h2>
        <p>Three simple steps to save any Twitch VOD, clip, or stream highlight with audio directly to your device.</p>
      </div>
      <div class="steps-grid">
        <div class="step-card">
          ...
          <h3>Copy Twitch Link</h3>
          <p>Open Twitch on the app or browser, click <strong>Share</strong> on any clip or VOD, and choose <strong>Copy Link</strong>.</p>
        </div>
        <div class="step-card">
          ...
          <p>Paste the Twitch link into the search box above. Select 1080p60, 720p HD with synced sound or MP3 audio.</p>
        </div>
        <div class="step-card">
  ...
  <section id="features" class="section section-alt">
    <div class="container">
      <div class="section-header">
        <h2>Why Choose 10. Twitch Video Downloader?</h2>
        <p>Engineered to quickly save Twitch VODs, clips, and highlights in high-definition quality.</p>
      </div>
      <div class="safety-grid">
        <div class="safety-card">
          <div class="safety-icon orange" style="background: var(--twitch-purple-light); color: var(--twitch-purple); border-color: var(--twitch-purple-border);">${ICONS.volume}</div>
          <div>
            <h4>HD Video with Clear Audio</h4>
            <p>Automatically captures and merges full stream video and audio into a crystal-clear MP4 file.</p>
          </div>
        </div>
        <div class="safety-card">
          <div class="safety-icon orange" style="background: var(--twitch-purple-light); color: var(--twitch-purple); border-color: var(--twitch-purple-border);">${ICONS.twitch}</div>
          <div>
            <h4>Supports Clips &amp; Full VODs</h4>
            <p>Full support for clips.twitch.tv, twitch.tv/videos/..., mobile Twitch links, and channel highlights.</p>
          </div>
        </div>
        <div class="safety-card">
        ...
        <div class="safety-card">
          <div class="safety-icon orange" style="background: var(--twitch-purple-light); color: var(--twitch-purple); border-color: var(--twitch-purple-border);">
  ...
  <section id="faq" class="section">
    <div class="container">
      <div class="section-header">
        ...
        <p>Everything you need to know about downloading Twitch videos, VODs, and clips with sound.</p>
      </div>
      <div class="faq-list">
        <details class="faq-item">
          <summary class="faq-q">
            <span>Can I download Twitch clips and full VODs?</span>
            ${ICONS.chevronDown}
          </summary>
          <p class="faq-a">Yes! 10. Twitch Video Downloader supports both short clips (clips.twitch.tv) and full VODs / past broadcasts in high-definition MP4 with crystal-clear audio.</p>
        </details>
        <details class="faq-item">
          <summary class="faq-q">
            <span>How do I copy a Twitch clip link?</span>
            ${ICONS.chevronDown}
          </summary>
          <p class="faq-a">On Twitch website or mobile app, click the Share or Clip icon on any stream or clip, click "Copy Link", and paste it into the downloader above.</p>
        </details>
        <details class="faq-item">
          <summary class="faq-q">
        ...
        <details class="faq-item">
          <summary class="faq-q">
            <span>Is 10. Twitch Video Downloader free to use?</span>
            ${ICONS.chevronDown}
          </summary>
  ...
  <div class="container">
    <div class="footer-top">
      <div class="footer-brand">
        ...
        <p class="footer-tagline">10. Twitch Video Downloader with Sound &bull; HD MP4 &amp; MP3</p>
      </div>
    ...
    <div class="footer-bottom">
      <p class="copyright-notice">
        Disclaimer: 10. Twitch Video Downloader is an independent web utility and is not affiliated, endorsed, or associated with Twitch Interactive, Inc. or Amazon. All trademarks belong to their respective owners.
      </p>
      <p class="copyright-year">&copy; ${year} 10. Twitch Video Downloader. All rights reserved.</p>
    </div>
  </div>
</footer>
`
...
function initInteractions(root) {
  ...
  if (urlInput) {
    urlInput.addEventListener('input', () => {
      ...
      if (isValidTwitchUrl(val)) {
        ...
        infoDebounce = setTimeout(async () => {
          try {
            ...
            if (data && (data.title || data.thumbnail)) {
              root.querySelector('#info-title').textContent = data.title || 'Twitch Video'
              if (data.uploader || data.channel) {
                root.querySelector('#info-author').textContent = data.uploader || data.channel
              }
        ...
        urlStatus.textContent = 'Please enter a valid Twitch video, VOD, or clip URL.'
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (!url) {
        errorText.textContent = 'Please enter a Twitch video or clip URL.'
        errorBox.hidden = false
        return
      }

      if (!isValidTwitchUrl(url)) {
        errorText.textContent = 'Please enter a valid Twitch video or clip URL (e.g. twitch.tv/videos/... or clips.twitch.tv/...).'
        errorBox.hidden = false
        return
      }
      ...
      try {
        ...
        if (!res.ok) {
          ...
          const msg = errData.message || FRIENDLY_ERRORS[res.status] || 'This Twitch video could not be downloaded.'
        ...
        if (data.success && data.download_url) {
          resultTitle.textContent = data.title || 'Twitch Video'
          downloadFileBtn.href = data.download_url
          downloadFileBtn.setAttribute('download', data.filename || 'twitch-video.mp4')
