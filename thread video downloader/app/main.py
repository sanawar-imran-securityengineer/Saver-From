...
from .downloader.threads_downloader import is_threads_url, resolve_threads_url
from .services.validation import validate_url, normalize_url, resolve_reddit_url, resolve_pinterest_url
...
app = FastAPI(
    title="ThreadSave",
    description="Fast public Meta Threads video downloader — MP4 and MP3. Public posts & videos only.",
    version="2.0.0",
)
...
async def video_info(request: Request, url: str):
    ...
    # Resolve Threads share links (threads.com/share/ID → canonical post URL)
    ...
    async def _do_info():
        if is_threads_url(url):
            try:
                return await manager._threads_downloader.extract_info(url)
            except Exception as e:
                logger.debug(f"Direct Threads info extraction failed, falling back: {e}")
        return await _ytdlp.extract_info(url)

    try:
        result = await asyncio.wait_for(
            _sf_info.do(cache_key, _do_info),
            timeout=25.0,
        )
        await _cache.set(cache_key, result, ttl=settings.cache_info_ttl)
        return result
    except asyncio.TimeoutError:
        logger.warning(f"Info extraction timed out for: {url}")
        raise HTTPException(status_code=504, detail="This video could not be retrieved in time.")
...
async def download_video(request: Request, body: DownloadRequest):
    """Download a public video in the selected format."""
    ...
    body.url = await resolve_pinterest_url(body.url)
    body.url = await resolve_reddit_url(body.url)
    # Resolve Threads share links (threads.com/share/ID → canonical post URL)
    if is_threads_url(body.url):
        body.url = await resolve_threads_url(body.url)
